...
b8 renderer_vulkan_instance_create(renderer_t *renderer, application_t *application) {
  ...
  logger_debug_format("<renderer:%p> <count:%lu> required instance layers", (void *)renderer, layers->count);
  ...
  logger_debug_format("<renderer:%p> <count:%lu> required instance extensions", (void *)renderer, extensions->count);
